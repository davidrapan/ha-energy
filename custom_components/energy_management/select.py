...
async def async_setup_entry(_: HomeAssistant, config_entry: ConfigEntry[Coordinator], async_add_entities: AddEntitiesCallback):
    ...
    async_add_entities([StrategySelectEntity(config_entry.runtime_data), StrategyNowSelectEntity(config_entry.runtime_data)])
...
class StrategySelectEntity(EnergyManagementSelectEntity):
    def __init__(self, coordinator):
        self._attr_key = "strategy"
        self._attr_name = "Strategy"
        self._attr_entity_category = EntityCategory.CONFIG
        self._attr_options = ["hourly", "daily_mean"]
        super().__init__(coordinator)

    @property
    def current_option(self):
        return self.coordinator.config_strategy

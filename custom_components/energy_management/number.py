...
class ChargePowerNumberEntity(EnergyManagementRestoreNumber):
    def __init__(self, coordinator):
        EnergyManagementRestoreNumber.__init__(self, coordinator)
...
class MinSOCNumberEntity(EnergyManagementRestoreNumber):
    ...
    async def async_added_to_hass(self):
        await super().async_added_to_hass()
        if last_number_data := await self.async_get_last_number_data():
            self.coordinator.number_soc_min = last_number_data.native_value

    def set_native_value(self, value: float):
        self.coordinator.number_soc_min = value
        self.async_write_ha_state()
...
class MaxSOCNumberEntity(EnergyManagementRestoreNumber):
    ...
    async def async_added_to_hass(self):
        await super().async_added_to_hass()
        if last_number_data := await self.async_get_last_number_data():
            self.coordinator.number_soc_max = last_number_data.native_value

    def set_native_value(self, value: float):
        self.coordinator.number_soc_max = value
        self.async_write_ha_state()
